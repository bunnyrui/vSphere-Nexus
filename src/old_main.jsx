...
const storageKey = "vsphere-nexus.form.v1";
const sessionStorageKey = "vsphere-nexus.session.v1";
const tokenKey = "vsphere-nexus.token";
...
function JobPanel({ job, onCancel, onRetry, onRefresh }) {
  ...
  useEffect(() => {
    ...
    async function connect() {
      ...
      const token = localStorage.getItem("vsphere-nexus.token");
  ...
  function exportLogs() {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `vsphere-nexus-${job.id}.log`;
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 200);
  }

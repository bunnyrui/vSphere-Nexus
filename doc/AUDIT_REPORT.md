...
| # | 严重度 | 问题 | 文件 | 状态 |
|---|---|---|---|---|
| I-1 | 🔴 | ~~`AlertCircle` 使用但未导入，打开运行中虚拟机的"修改配置"模态框触发 `ReferenceError` 崩溃~~ | `InventoryPage.jsx:1043` | ✅ 补充导入，移除不可达的警告提示（入口已拦截） |
...
| # | 严重度 | 问题 | 文件 | 状态 |
|---|---|---|---|---|
| DP-1 | 🔴 | ~~部署数量输入无上限，输入 999999 分配百万元素数组，冻结浏览器~~ | `DeploymentPage.jsx:392-395` | ✅ 前端软限制（预览报错+禁用下一步+提交校验），后端 vms.length≤100 |
| DP-2 | 🔴 | ~~第 2→3 步无任何验证，可不填命名规则和网络映射直接到确认页~~ | `DeploymentPage.jsx:463-468` | ✅ 前缀必填+网络映射必选校验，未通过禁用下一步 |
...
| BE-ROB-2 | 🔴 | ~~任务无所有权校验~~ | `server/jobs.js` | ✅ 设计决策：单用户工具，不需要多租户隔离 |

...
vSphere Nexus 是一个基于 Web 的 vSphere 虚拟机批量部署与清理工具。它通过 `ovftool` 命令行工具与 ESXi/vCenter 交互，提供以下功能：
...
│   ├── routes/                 # API 路由
│   ├── services/               # 业务服务层
│   │   ├── vimClient.js        # SOAP 基础客户端：处理 XML 封装、TLS 绕过、Cookie 管理
│   │   └── vmService.js        # 业务逻辑服务：清单发现、电源控制、快照管理
...
```
vSphere Nexus server listening on http://localhost:4173
ovftool: /opt/vSphere Nexus/bin/linux/ovftool
```
...
```bash
# 1. 克隆项目
git clone <repo-url> /opt/vSphere Nexus
cd /opt/vSphere Nexus
...
```bash
sudo useradd -r -s /bin/false -d /opt/vSphere Nexus vsphere-nexus
sudo chown -R vsphere-nexus:vsphere-nexus /opt/vSphere Nexus
```

#### 2. 创建 systemd 服务文件

创建 `/etc/systemd/system/vsphere-nexus.service`：

```ini
[Unit]
Description=vSphere Nexus - vSphere Batch VM Deployment Tool
Documentation=https://github.com/your-org/vSphere Nexus
...
User=vsphere-nexus
Group=vsphere-nexus
WorkingDirectory=/opt/vSphere Nexus
...
Environment=NEXUS_USER=admin
Environment=NEXUS_PASS=your-secure-password-here
...
ProtectHome=true
ReadWritePaths=/opt/vSphere Nexus/data

# 日志
StandardOutput=journal
StandardError=journal
SyslogIdentifier=vsphere-nexus
...
# 设置开机自启
sudo systemctl enable vsphere-nexus

# 启动服务
sudo systemctl start vsphere-nexus

# 查看状态
sudo systemctl status vsphere-nexus

# 查看日志
sudo journalctl -u vsphere-nexus -f
```
...
```bash
# 停止服务
sudo systemctl stop vsphere-nexus

# 重启服务
sudo systemctl restart vsphere-nexus

# 查看最近 100 行日志
sudo journalctl -u vsphere-nexus -n 100 --no-pager
```
...
```bash
docker build -t vsphere-nexus:latest .
```
...
docker run -d \
  ...
  -p 4173:4173 \
  -e NEXUS_USER=admin \
  -e NEXUS_PASS=your-secure-password \
  -v vsphere-nexus-data:/app/data \
  vsphere-nexus:latest
```
...
```bash
docker compose up -d
docker compose logs -f vsphere-nexus
```
...
创建 `/etc/nginx/sites-available/vsphere-nexus`：

```nginx
server {
    listen 80;
    server_name vsphere-nexus.example.com;
...
server {
    ...
    # SSL 证书
    ssl_certificate     /etc/ssl/certs/vsphere-nexus.crt;
    ssl_certificate_key /etc/ssl/private/vsphere-nexus.key;
...
```bash
sudo ln -s /etc/nginx/sites-available/vsphere-nexus /etc/nginx/sites-enabled/
...
| `NEXUS_USER` | - | 否 | Web 界面认证用户名（需与 `NEXUS_PASS` 同时设置） |
| `NEXUS_PASS` | - | 否 | Web 界面认证密码 |
...
设置 `NEXUS_USER` 和 `NEXUS_PASS` 环境变量即可启用 Web 认证：

```bash
export NEXUS_USER=admin
export NEXUS_PASS='Str0ng!P@ssw0rd'
npm start
```
...
```json
{
  "ok": true,
  "authEnabled": true,
  "ovftoolPath": "/opt/vSphere Nexus/bin/linux/ovftool",
...
# 手动清理（停止服务后操作）
systemctl stop vsphere-nexus
# 编辑 data/jobs.json 删除旧任务，然后重启
systemctl start vsphere-nexus
```

### 优雅关闭

vSphere Nexus 支持 SIGTERM/SIGINT 信号优雅关闭：
...
```bash
# 优雅关闭
sudo systemctl stop vsphere-nexus   # systemd 会发送 SIGTERM
...
- ESXi/vCenter 服务未运行
- IP 地址或域名错误
- 运行 vSphere Nexus 的机器不在同一网络/VPN
